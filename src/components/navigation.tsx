    ...
    <header className={`glass-light fixed inset-x-0 top-0 z-50 border-x-0 border-t-0 transition-shadow ${scrolled ? "shadow-[0_12px_35px_-24px_rgba(41,40,37,.55)]" : ""}`}>
      ...
      <div className="container-shell flex h-[72px] items-center justify-between">
        ...
        <nav aria-label="Main navigation" className="hidden items-center gap-6 lg:flex">
          {links.map(([label, href]) => <a key={href} href={href} className="focus-ring py-2 text-[.7rem] font-semibold uppercase tracking-[.1em] text-muted underline-offset-8 transition-colors hover:text-gold hover:underline hover:decoration-gold/40">{label}</a>)}
        </nav>
        <div className="hidden items-center gap-3 lg:flex">
          ...
          <a href="#join" className="focus-ring bg-charcoal px-5 py-3 text-xs font-bold uppercase tracking-widest text-cream hover:bg-gold">Join Nyoni</a>
        </div>
      ...
      <div id="mobile-menu" className={`fixed inset-x-0 top-[72px] h-[calc(100dvh-72px)] bg-cream px-5 py-6 transition duration-200 sm:top-[104px] sm:h-[calc(100dvh-104px)] lg:hidden ${open ? "visible opacity-100" : "invisible opacity-0"}`}>
        <nav aria-label="Mobile navigation" className="mx-auto flex max-w-xl flex-col border-t border-charcoal/15">
          {links.map(([label, href]) => <a key={href} href={href} onClick={() => setOpen(false)} className="border-b border-charcoal/15 py-4 font-display text-3xl uppercase transition-colors hover:text-gold">{label}</a>)}
          <a href="#join" onClick={() => setOpen(false)} className="mt-6 bg-charcoal px-5 py-4 text-center text-sm font-bold uppercase tracking-widest text-cream hover:bg-gold">Join Nyoni</a>
