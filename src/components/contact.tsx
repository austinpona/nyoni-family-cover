...
import { EMAIL, PHONE_DISPLAY, PHONE_E164, businessHours, company, whatsappUrl } from "@/lib/site-data";
    ...
    <section id="contact" className="paper-grain bg-cream pb-20 sm:pb-28">
      <Reveal className="container-shell">
        <div className="grid border-y border-charcoal/20 py-12 lg:grid-cols-[1.15fr_.85fr] lg:items-end">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:mt-0"><a href={whatsappUrl("Hello Nyoni, I would like to speak to your team.")} target="_blank" rel="noreferrer" className="focus-ring flex min-h-12 items-center justify-center gap-3 bg-charcoal px-5 py-4 text-xs font-bold uppercase tracking-widest text-cream hover:bg-gold"><MessageCircle size={18} />WhatsApp</a><a href={`tel:${PHONE_E164}`} className="focus-ring flex min-h-12 items-center justify-center gap-3 border border-charcoal/30 px-5 py-4 text-xs font-bold uppercase tracking-widest hover:border-gold hover:text-gold"><Phone size={18} />Call now</a><a href={`mailto:${EMAIL}`} className="focus-ring flex min-h-12 items-center justify-center gap-3 border border-charcoal/30 px-5 py-4 text-xs font-bold uppercase tracking-widest hover:border-gold hover:text-gold sm:col-span-2"><Mail size={18} />Email</a></div>
        </div>
        <div className="grid gap-6 pt-8 sm:grid-cols-2 lg:grid-cols-4"><div className="flex gap-3"><Phone size={18} className="mt-1 text-gold" /><div><p className="text-xs font-bold uppercase tracking-wider">Contact</p><a href={`tel:${PHONE_E164}`} className="mt-1 block text-sm">{PHONE_DISPLAY}</a><a href={`mailto:${EMAIL}`} className="mt-1 block break-all text-sm text-muted underline underline-offset-2 hover:text-gold">{EMAIL}</a></div></div>{/* Both facts here are verified: the operating area from the CIPC
