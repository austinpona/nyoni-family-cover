...
export function Hero() {
  ...
  const entrance = { initial: false as const };
  ...
  return (
    <section id="home" className="relative overflow-hidden bg-slate pt-[72px] text-cream sm:pt-[104px]">
      ...
      <div className="container-shell relative grid min-h-[calc(100dvh-72px)] items-center gap-10 py-12 sm:min-h-[calc(100dvh-104px)] sm:py-16 lg:grid-cols-[1.08fr_.92fr]">
        <motion.div className="relative z-10 min-w-0 max-w-3xl" {...entrance} transition={{ duration: .75, ease: [0.22, 1, 0.36, 1] }}>
          ...
          <h1 className="display-title text-[clamp(3.05rem,14vw,7rem)] leading-[.88] text-white sm:text-[clamp(3.75rem,7.5vw,7rem)]">Practical support.<span className="mt-2 block text-[#e2c99c]">When it matters.</span></h1>
        ...
        <motion.aside initial={false} className="support-ledger relative mx-auto min-w-0 w-full max-w-[29rem] overflow-hidden rounded-xl p-6 text-white sm:p-8" aria-label="Basic membership support summary">
          <div className="flex items-center justify-between border-b border-white/15 pb-5"><div><p className="text-[.62rem] font-bold uppercase tracking-[.18em] text-white/55">Included support</p><p className="mt-2 font-display text-3xl uppercase">Basic benefits</p></div><div className="relative size-16 shrink-0 overflow-hidden rounded-full border border-light-gold/35 bg-white"><Image src="/images/nyoni-logo.png" width={1100} height={1430} priority sizes="64px" alt="" className="absolute left-1/2 top-0 w-[105%] max-w-none -translate-x-1/2" /></div></div>
          <ol className="divide-y divide-white/12">{["One cow", "2 × 50kg maize meal", "One full load of firewood", "One-day bakkie service"].map((label, index) => <li key={label} className="flex items-center justify-between gap-4 py-4"><span className="text-sm font-medium text-white/88">{label}</span><span className="font-display text-xl text-light-gold">0{index + 1}</span></li>)}</ol>
          <p className="border-t border-white/15 pt-5 text-xs leading-5 text-white/58">Benefits apply after the six-month waiting period and successful verification.</p>
        </motion.aside>
      </div>
    </section>
  );
